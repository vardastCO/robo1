...
(async () => {
    ...
    async function processPage(pageUrl) {
        ...
        try {
            await page.goto(pageUrl, { timeout: 60000 }); // Increased timeout to 60 seconds
            ...
            for (const href of hrefs) {
                if (href && !processedHrefs.has(href)) {
                    const outputUrl = href.startsWith('https://') ? href : initialPage + href;
            ...
            while (retryCount < maxRetries) {
                try {
                    ...
                    if (error.name === 'TimeoutError') {
                        ...
                        retryCount++;
                        await pageForEvaluation.waitForTimeout(3000); // Wait for 3 seconds between retries
                    } else {
